...
A 160-example synthetic benchmark (`tools/rstf_benchmark.py`) measures detection/recovery accuracy (95.0% overall, 0% false positives on unmodified text) — but that benchmark is circular by construction (it's generated by the same tables the detector uses). Two independent checks exist for that: `tools/rstf_adversarial_eval.py` runs hand-authored/independently-recalled examples (91.7% detection, 0% false positives on legitimate non-English text), and `tools/rstf_dogfood_scan.py` scans this repo's own real `.md`/`.py` files (not a customer pilot — no external party supplied the data) for false positives. All three evals found and fixed real bugs: the homoglyph detector previously flagged all monolingual Cyrillic/Greek text as an attack (100% false positive); the reversed-text detector flagged ordinary code like `import os` (19 unrelated hits in this repo alone); and, found on a later rerun of the dogfood scan, the same detector flagged real prose repeating "no" (which reverses into the also-common word "on") — see `docs/SEMANTIC_TRANSFORM_FINGERPRINT.md` for what changed and the residual limitations of each fix.

`tools/rstf_token_cost.py` measures whether canonicalization actually reduces UTF-8 byte length before it reaches a model. This is an offline byte upper-bound proxy, not a billable tokenizer count.

Beyond the byte proxy, `tools/bpe_tokenizer.py` trains a real byte-level BPE tokenizer (the same algorithm GPT-2/3.5/4, Claude, and Llama use) on this repo's own docs, with a C++ mirror in `cpp/src/tokenizer.cpp` (replacing an earlier single-character-lookup stub) parity-verified against the Python reference by `tools/bpe_parity_check.py`. `tools/rstf_bpe_token_cost.py` measures real token counts with it: 66.2% overall token savings, and — the finding the byte proxy structurally cannot produce — **38.0% savings on plain reversed text**, because BPE merges are learned from forward-reading corpus text and reversing a string breaks them, unlike byte count, which cannot change under reordering. This is OverLLM's own tokenizer trained on a few tens of KB of this repo's own text, not GPT-4/Claude/Llama's production tokenizer — absolute counts aren't production billing, but the algorithm and measurement are genuine, not a proxy.

`tools/rstf_tiktoken_cost.py` adds a production-tokenizer path when `tiktoken` is installed: it measures real tiktoken counts for a selected OpenAI model or encoding (`--model` or `--encoding`) and can render JSON/Markdown reports. It does **not** hard-code provider pricing; optional cost estimates require caller-supplied per-1M-token input pricing through `--input-price-per-1m`. Truth label: `real_tiktoken_count_for_selected_model_or_encoding_not_provider_bill`.
...
Read the full guides: [`docs/SEMANTIC_TRANSFORM_FINGERPRINT.md`](docs/SEMANTIC_TRANSFORM_FINGERPRINT.md) and [`docs/RSTF_TIKTOKEN_COST.md`](docs/RSTF_TIKTOKEN_COST.md).

Important truth label: the glyph and homoglyph tables are curated subsets, not exhaustive Unicode confusables data, and the bidi recovery is a simplified reconstruction, not a full UAX#9 implementation. The byte-cost benchmark reports UTF-8 byte length, not real tokenizer token counts. The real-BPE benchmark closes that gap with OverLLM's own tokenizer, not a production one. The tiktoken benchmark reports real tiktoken counts for a selected OpenAI model/encoding, not universal provider billing. Neither the adversarial eval nor the dogfood scan is a substitute for a real external pilot — no party outside this repo has tested RSTF on their own data yet.

---

## Real transformer learning

`cpp/src/model.cpp` is a genuine decoder-only transformer — causal multi-head self-attention, learned positional embeddings, GELU FFN, post-LN residual blocks — the same architecture family as GPT. Its backward pass, however, had never been numerically verified, and turned out to have seven distinct, compounding gradient-correctness bugs: attention `Q`/`K`/`V` backprop was a stub that never actually computed real gradients (gradient into the embedding table skipped attention entirely), both LayerNorm backward calls used the wrong input, a GELU backward call read the wrong buffer, `Wo` and `output_proj`'s weight gradients used the wrong activations, and no bias gradient was ever accumulated for any attention or FFN sublayer. Loss still decreased in earlier runs of this experiment — that was real, just not proof the fix below wasn't still needed, since it was propelled by whichever paths happened to be correct, not by attention learning anything.

Fixed all seven, and added `cpp/tools/gradient_check.cpp` — a numerical gradient check (finite-difference vs. analytical, `torch.autograd.gradcheck`-style tolerance) that verifies the corrected backward pass across every sublayer at non-last sequence positions specifically, since the causal-accumulation bug only shows up there. All 30 probes pass. `cpp/tools/train_language_model.cpp` (run via `python tools/transformer_learning_check.py`) now trains via the new `overllm_forward_seq`/`overllm_backward_seq` API — real per-position next-token loss across whole context windows, the way real GPT pretraining batches contiguous context — on real BPE-tokenized text: loss goes 7.43 → 4.70 over 30 epochs (36.7% reduction, stronger than the pre-fix run's 29.8% now that attention actually learns), no NaN, monotonically decreasing every epoch.

Building the original version of this found a separate real bug: run through this repo's actual `-O3 -ffast-math` CMake build (not a looser standalone compile), training diverged to NaN around epoch 15, because `model.cpp` had no gradient clipping anywhere in its training path — a standard safeguard every real transformer training setup uses. Added `overllm_clip_gradients()` to fix it; this remains necessary and unchanged by the backward-pass fix above. Full writeup, including the honest limits (tiny corpus, no held-out eval, memorization not generalization): [`docs/TRANSFORMER_LEARNING.md`](docs/TRANSFORMER_LEARNING.md).
